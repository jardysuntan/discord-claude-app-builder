...
MAX_AGENT_MEMORY = 20          # keep last N entries per (user_id, workspace) key
...
# (user_id, ws_key) -> bool; default True
_memory_enabled: dict = {}
...
async def run_agent(
    ...
    mem_enabled = _memory_enabled.get(_mem_key(user_id, ws_key), True)
    memory = _agent_memory.get(_mem_key(user_id, ws_key), []) if mem_enabled else []
...
def parse(text: str, default_ws: str):
    ...
    if t.startswith("/memory"):
        sub = t[len("/memory"):].strip().lower()
        if sub in ("on", "off", "reset", "show"):
            return ("__CMD__", f"memory:{sub}")
        return ("__CMD__", "memory:?")
...
async def on_message(message: discord.Message):
    ...
    if ws == "__CMD__":
        ...
        if payload.startswith("memory:"):
            ...
            ws_key = default_ws or ""
            mem_key = _mem_key(user_id, ws_key)

            if sub == "on":
                _memory_enabled[mem_key] = True
                await message.channel.send(f"✅ Memory ON for workspace **{ws_key or '(none)'}**.")
                return

            if sub == "off":
                _memory_enabled[mem_key] = False
                await message.channel.send(f"✅ Memory OFF for workspace **{ws_key or '(none)'}**.")
                return
            ...
            if sub == "reset":
                ...
                if ws_key and ws_key in workspaces:
                    save_agent_memory(user_id, ws_key, Path(workspaces[ws_key]).expanduser())
                await message.channel.send(f"✅ Memory reset for workspace **{ws_key or '(none)'}**.")
                return

            if sub == "show":
                if ws_key and ws_key in workspaces:
                    load_agent_memory(user_id, ws_key, Path(workspaces[ws_key]).expanduser())
                entries = _agent_memory.get(mem_key, [])
                if not entries:
                    await message.channel.send("No memory entries yet.")
                else:
                    shown = entries[-10:]
                    lines = [f"Memory (last {len(shown)}/{len(entries)} entries):"]
                    for i, e in enumerate(shown, len(entries) - len(shown) + 1):
                        lines.append(f"{i}. [{e['user'][:80]}] → {e['summary'][:120]}")
                    await message.channel.send("\n".join(lines))
                return

            await message.channel.send("Usage: /memory on | off | reset | show")
            return
    ...
    user_id = message.author.id
    mem_key_ws = _mem_key(user_id, ws)
    load_agent_memory(user_id, ws, repo_path)
    mem_enabled_ws = _memory_enabled.get(mem_key_ws, True)
    ws_memory = _agent_memory.get(mem_key_ws, []) if mem_enabled_ws else []
    if ws_memory:
        mem_lines = ["You are a helpful coding assistant. Below is context from past sessions.", ""]
        for entry in ws_memory:
            mem_lines.append(f"[USER]: {entry['user']}")
            mem_lines.append(f"[ASSISTANT SUMMARY]: {entry['summary']}")
        mem_lines.append("")
        mem_lines.append(f"[USER]: {payload}")
        full_prompt = "\n".join(mem_lines)
    else:
        full_prompt = payload
    reply = await run_claude(full_prompt, repo_path)
    if mem_enabled_ws:
        append_agent_memory(user_id, ws, payload, reply[:300])
        save_agent_memory(user_id, ws, repo_path)
