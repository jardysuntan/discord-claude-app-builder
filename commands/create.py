...
async def create_kmp_project(app_name: str, registry: WorkspaceRegistry, owner_id: int | None = None,
    ...
    claude_md.write_text(
        ...
        f"## Data-Driven Content\n"
        f"- NEVER hardcode display content (dates, categories, labels, amenity lists) in composables.\n"
        f"- All screen content must come from the data model loaded via ConfigRepository (e.g. `config.categories`, `config.startDate`).\n"
        f"- Create a metadata or config data class that holds trip/event/app-level info (name, tagline, dates, location) and derive all display values from it.\n"
        f"- Build date-utility functions (e.g. `formatDateRange(start, end)`, `generateDayList(start, end)`) in a shared `DateUtil` object — screens should call these instead of computing dates inline.\n"
        f"- Schedule or time-based logic must be parametric — derive from config dates, never from hardcoded timestamps.\n"
        f"- Fallback demo data is fine, but it must populate the same @Serializable data classes used for real data.\n\n"
        ...
        f"|-----------|-----|-------------------|-------------|\n"
        f"| Maps | No CMP maps component — use PlatformMapView expect/actual (see Maps section below) | PlatformWebViewHtml + Leaflet (inline base64) | PlatformWebViewHtml + Leaflet (inline base64) |\n"
        ...
        f"## Maps (Leaflet.js — platform-specific)\n"
        f"Maps use the `PlatformMapView` expect/actual pattern with Leaflet.js.\n"
        f"Map tiles load from `https://{{s}}.tile.openstreetmap.org/{{z}}/{{x}}/{{y}}.png` at runtime.\n\n"
        f"### Shared: `platform/LeafletMapHtml.kt` (commonMain)\n"
        f"- `buildLeafletHtml(venues)` generates a self-contained HTML page with Leaflet\n"
        f"- Leaflet JS/CSS are inlined via `LeafletBundle.jsBase64` and `LeafletBundle.css` — NO CDN `<script src>` (blocked by WKWebView)\n"
        f"- JS loads Leaflet with `eval(atob(\"...\"))` which is guaranteed synchronous\n"
        f"- HTML attributes inside JS strings must use double quotes (`style=\"...\"`) — single quotes break the enclosing JS string\n\n"
        f"### Android actual\n"
        f"- Delegates to `PlatformWebViewHtml(html, modifier)` using `buildLeafletHtml(venues)`\n"
        f"- **Height fix required:** Android WebView's `loadDataWithBaseURL` doesn't resolve `height:100%`. Inject:\n"
        f"  `<script>document.getElementById('map').style.height=window.innerHeight+'px';</script>` after the map div\n\n"
        f"### iOS actual\n"
        f"- Delegates to `PlatformWebViewHtml(buildLeafletHtml(venues), modifier)` — works as-is with WKWebView\n\n"
        f"### Web (WASM) actual — CRITICAL DIFFERENCES\n"
        f"Iframes do NOT work in Compose WASM (`srcdoc`, `document.write`, script injection all fail). Instead:\n"
        f"1. Use a `@JsFun` JS function that takes `(containerId, venuesJson)` parameters\n"
        f"2. Create the map div directly in the DOM container (no iframe, no HTML parsing)\n"
        f"3. Load Leaflet CSS/JS from CDN dynamically (`document.createElement('script')`)\n"
        f"4. Wait for BOTH CSS `onload` AND JS `onload` before calling `initMap()`\n"
        f"5. Parse the JSON venues and call `L.map()`, `L.marker()`, `L.tileLayer()` directly\n"
        f"6. Add `crossOrigin: true` to `L.tileLayer()` options (required for COEP headers)\n"
        f"7. Use `ResizeObserver` on the container to call `map.invalidateSize()` on resize\n"
        f"8. Pass venue data from Kotlin via `Json.encodeToString(venues)` → `toJsString()`\n\n"
        f"See `platform/PlatformWebView.wasmJs.kt` and `platform/LeafletMapHtml.kt` for working examples.\n\n"
        f"## Supabase\n"
        ...
        f"- If this app has an assigned schema (see workspace config), pass `--schema <name>` to the script\n"
        f"- The schema name is stored in the workspace config — it isolates this app's tables from other apps\n"
        f"- When writing SQL, always include `SET search_path TO <schema>, public;` at the top if a schema is assigned\n"
